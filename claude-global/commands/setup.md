...
- Folders, brains and workflows come next and are separate jobs. Add them to `TASKS.md` under `## Next`: create `~/personal`, `~/projects/_scratch`, `~/products`; clone or create `~/vault` from `templates/vault`; `~/archive`, `~/library/camera`, `~/resources` brains from `templates/`; `schedule install` for tasks-sync, feeds-sync, the daily note and map-check. Tell me what is left.
