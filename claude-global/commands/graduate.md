...
3. Fix anything that pointed at the old path: `rg -l "projects/$ARGUMENTS"` across `~/system-manager`, `~/vault`, and the project itself; any launchd plists that reference it (`rg -l` in `~/Library/LaunchAgents`); VS Code workspaces.
