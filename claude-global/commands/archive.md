...
3. Run `~/system-manager/bin/archive-push <paths> --dry-run`, then — **after I say yes** — `archive-push <paths> --delete-local`.
...
5. If a project repo was archived, remove it from `~/system-manager/docs/FLEET.md`/maps if listed, mark its vault hub `status: archived`, and run `tasks-sync`.
