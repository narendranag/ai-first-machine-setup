...
Linked into `~/.claude/` from `~/system-manager/claude-global/`. Edit it there, never in `~/.claude`.
...
- Name: {{user_name}}
- GitHub: {{github_user}}
- I work in Python and TypeScript. How I build: `~/system-manager/docs/DEV-GUIDELINES.md`.
- My machines, their roles and how to reach them: `~/system-manager/docs/FLEET.md`. This machine's state: `~/system-manager/docs/machines/<host>.md`.

## How you work with me

- Start where the work is. A project's `CLAUDE.md` says what to do there; `~/system-manager/docs/` says what the system looks like.
...
- **After any change to a machine** (install, service, port, launchd job, key), update its file in `~/system-manager/docs/machines/` in the same session.
- **Record decisions** in `~/system-manager/docs/DECISIONS.md` (system) or the project's `DECISIONS.md`: `- [<host>] [YYYY-MM-DD] <decision> — <why>`.
